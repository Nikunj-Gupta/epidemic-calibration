...
class SIRVDModel:
    ...
    def load_config(self, config_name):
        ...
        I0 = float(data[1])
        S0 = N-I0
        y0 = S0, I0, float(data[2]), float(data[3]), float(data[4])
        data_info = data[5:9]

        guess = [float(x) for x in data[9:14]]
    ...
    #SIR EDO
    def deriv(self, y0, t, N, beta, gamma, alpha, sigma, delta): 

        S, I, R, V, D = y0
        #print(y0)
        
        dy = np.zeros(5)
        dy[0] = -beta * S * I / N + sigma * R - alpha * S  #dSdt 
        dy[1] = beta * S * I / N - gamma * I - delta * I #dIdt 
        dy[2] = gamma * I - sigma * R #dRdt 
        dy[3] = alpha * S   #dVdt 
        dy[4] = delta * I   #dDdt 
        return dy

    def SIRSolve(self, y0, t, N, beta, gamma, alpha, sigma, delta):

        res = odeint(self.deriv, y0, t, args=(N, beta, gamma, alpha, sigma, delta)) 
        S, I, R, V, D = res.T
    ...
    def err(self, params, t,  data_i, data_d, y0):
        ...
        S, I, R, V, D  = self.SIRSolve(y0, t, N, beta, gamma, alpha, sigma, delta)
    ...
    def fit(self , config_name, methods='leastsq', max_nfev=100000):
        ...
        params.add('beta',value=guess[0],min=0, max = 10, vary=True)
        params.add('gamma',value=guess[1],min=0, max=2, vary=True)
        params.add('alpha',value=guess[2],min=0, max=2, vary=True)
        params.add('sigma',value=guess[3],min=0, max=2, vary=True)
        params.add('delta',value=guess[4],min=0, max=2, vary=True)
        params.add('N', value=N, vary=False)
        ...
        sigma_fit=out.params['sigma'].value
        delta_fit=out.params['delta'].value
        fitted_parameters=([beta_fit, gamma_fit, alpha_fit, sigma_fit, delta_fit])
        fitted_curve=self.SIRSolve(y0, t, N, beta_fit, gamma_fit, alpha_fit, sigma_fit, delta_fit) 
        mae = mean_absolute_error(data_nr, fitted_curve[:][1])
